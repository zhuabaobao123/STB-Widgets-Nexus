...
#pragma warning(push)
#pragma warning(disable: 5054)  // operator '|' between different enumeration types
...
namespace ImGuiMCP {
    // typedef struct ImVec2 ImVec2;  // removed: conflicts with struct definition
    struct ImVec2 {
        float x, y;
    };
...
#define IM_COL32_BLACK_TRANS IM_COL32(0, 0, 0, 0)    // Transparent black = 0x00000000
    ...
    // typedef struct ImVec4 ImVec4;  // removed: conflicts with struct definition
    struct ImVec4 {
        float x, y, z, w;
    };
    typedef enum {
    ...
    // typedef struct ImDrawCmdHeader ImDrawCmdHeader;  // removed
    ...
    // typedef struct ImFontAtlasCustomRect ImFontAtlasCustomRect;  // removed
...
#pragma endregion
    ...
    inline void ImGuiTextBufferAppend(ImGuiTextBuffer* buffer, const char* fmt, ...) {
        using func_t = void (*)(ImGuiTextBuffer*, const char*, va_list);
        func_t func = reinterpret_cast<func_t>(GetProcAddress(menuFramework, "ImGuiTextBuffer_appendfv"));
        va_list args;
        va_start(args, fmt);
